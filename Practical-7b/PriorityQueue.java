...
public class PriorityQueue <T>{
	 private ArrayList<T> heap;   
	    ...
	    public void enqueue(T item) {
	        ...
	        siftUp(heap.size() - 1); // Adjust the heap from bottom to top
	    }
	    public T dequeue() {
	        if (isEmpty()) {
	            throw new NoSuchElementException("Priority Queue is empty");
	        }
	        T root = heap.get(0); // The element with the highest priority (min)
	    ...
	    public T peek() {
	        ...
	        return heap.get(0);  // Root element has the highest priority
	    }
